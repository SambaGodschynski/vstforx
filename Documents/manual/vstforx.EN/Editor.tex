...
\ukap{\label{corona}The Corona Field}
...
\begin{description}
	\item[The Node Corona]  (figure \ref{nodec}) The node corona which is a single (yellow) field around a node. With draggin this yellow zone you are able to move the node around. Connecting will be approached by dragging the mouse over the node itself.
...
Similar to real hardware you are able to connect arbitrary inputs with outputs.
\paragraph{Main Entry} (\includegraphics[scale=0.5, bb = 0 15 40 0]{../graphics/InputNode}) \\\\
...
\paragraph{Main Exit} (\includegraphics[scale=0.5,bb = 0 15 40 0]{../graphics/OutputNode}) \\\\
