...
void VSTPlugin::setupFramesbuffer() {
	...
	for ( size_t i=0; i<getNumOutputNodes()*2; i+=2 ) {
		::processing::Frames *fr = &( framebuffer[i/2] );
...
void VSTPlugin::initParameter(){
	...
	for ( size_t i=0; i<param.size(); i++ ){
		...
		param[i]->setMin( (::com::VstNumber)INT_MIN ); //entferne min, max ( siehe issue: 0000049 )
		param[i]->setMax( (::com::VstNumber)INT_MAX );
		// hole Parameter wert
		param[i]->setValue ( aEff->getParameter ( aEff, i ) );
		// hole Parameter name
		aEff->dispatcher ( aEff, effGetParamName, i, NULL, &bff[0], NULL );
		param[i]->setName ( com::MyString(bff) );
		// hole Parameter label
		aEff->dispatcher ( aEff, effGetParamLabel, i, NULL, &bff[0], NULL );
		param[i]->setLabel ( ::com::MyString(bff) );
		// hole Parameter Display
		aEff->dispatcher ( aEff, effGetParamDisplay, i, NULL, &bff[0], NULL );
		param[i]->setDisplay( ::com::MyString(bff) );
		// add listener
...
void VSTPlugin::processAdapter( ::processing::Processor::Int numSamples ) {
	// breite daten vor ( mappe frames => matrix )
    size_t c=0;
	for ( size_t i=0; i<getNumInputNodes(); ++i ) {
		::processing::ProcessorNode::Ptr pr = getInputNode(i);
		...
		::processing::Frames *fr = pr->popFrame();    
		inMatrix[c++] = (*fr)[0];
		inMatrix[c++] = (*fr)[1];
	}
...
void VSTPlugin::save(com::oArchive &ar, const unsigned int version) const {
	...
	// save plugInfo
	const ::processing::PluginInfo plugInfo = getPluginInfo();
	ar << plugInfo; //......................................................................................2
...
void VSTPlugin::load(com::iArchive &ar, const unsigned int version) {
	...
	//check type
	if ( plugInfo.pluginType != ::processing::PluginInfo::VST2X )
...
//------------------------------------------------------------------------------------------------------------
void VSTPlugin::getShellPluginInfos(::processing::ShellPluginInfos &out) {
	...
	while ((plugUniqueID = aEff->dispatcher (aEff, effShellGetNextPlugin, 0, 0, tempName, 0)) != 0) { 
		...
		if (tempName[0] != 0) {
			out.push_back(::processing::ShellPluginInfo(std::string(tempName), plugUniqueID));
		}
	}
}
//------------------------------------------------------------------------------------------------------------
